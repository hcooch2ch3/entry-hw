...
Module.prototype._init = function() {
    ...
    // 재연결(웹소켓 재접속 → reset() 경유)에서만 _primed=false로 두어, 브라우저가 유지한 stale
...
Module.prototype.handleLocalData = function(data) {
    ...
    // soundState는 _sound_event로 추가 게이트: 우리가 낸 소리의 완료만 노출한다.
...
Module.prototype.reset = function() {
    ...
    // (아래 줄은 반드시 this._init() 뒤에 와야 한다 — _init이 _primed=true로 재설정하므로.)
    this._primed = false;
    var s = this.sensory;
    for (var k in s) {
